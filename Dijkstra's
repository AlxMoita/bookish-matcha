...
void * receive_info (void *arg){ 
    ...
    while(1) { 
        ...
        int sender = ntohl(packet[0]); 
        int neighbor = ntohl(packet[1]); 
        int new_cost = ntohl(packet[2]); 

        pthread_mutex_lock(&lock);  
        costs[sender][neighbor] = new_cost; 
        costs[neighbor][sender] = new_cost; 
        pthread_mutex_unlock(&lock); 

        printf("New cost from %d to %d: %d\n", sender, neighbor, new_cost); 
        print_costs(); 
...
if (taken[j] == 0){
pthread_mutex_lock (&lock);
distances[j] = MIN (distances[j], (distances[spot] +
costs[spot][j]));
pthread_mutex_unlock (&lock);
        }
    }
...
for (i = 0; i < nodes; i++){
fscanf (fp, "%s%s%d", routers[i].name, routers[i].ip,
&routers[i].port); 
}
fclose (fp);
// get costs
...
inet_pton (AF_INET, routers[j].ip,
&otheraddr.sin_addr.s_addr);
